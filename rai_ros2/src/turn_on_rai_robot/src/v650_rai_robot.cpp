...
#include "rai_robot_msg/msg/data.hpp"     // CHANGE
...
: rclcpp::Node ("rai_robot")
{
  ...
  distance_publisher = create_publisher<rai_robot_msg::msg::Supersonic>("Distance", 1);//超声波数据
  ...
  RCLCPP_INFO(this->get_logger(),"rai_robot Data ready"); //Prompt message //提示信息
  ...
  {
    RCLCPP_ERROR(this->get_logger(),"rai_robot can not open serial port,Please check the serial port cable! "); //If opening the serial port fails, an error message is printed //如果开启串口失败，打印错误信息
  }
  if(Stm32_Serial.isOpen())
  {
    RCLCPP_INFO(this->get_logger(),"rai_robot serial port opened"); //Serial port opened successfully //串口开启成功提示
  }
}
/**************************************
Date: January 28, 2021
