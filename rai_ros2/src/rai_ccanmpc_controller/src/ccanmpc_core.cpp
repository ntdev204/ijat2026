  ...
  const nav_msgs::msg::OccupancyGrid * costmap)
{
  ...
  const auto vx_values = linspace(-0.30 * result.vx_max, result.vx_max, params_.vx_samples);
  const auto vy_values = linspace(-result.vy_max, result.vy_max, params_.vy_samples);
  const auto omega_values = linspace(-result.omega_max, result.omega_max, params_.omega_samples);
  ...
  for (double vx_raw : vx_values) {
    if (timeout_break) {
      break;
    }
    for (double vy_raw : vy_values) {
      if (timeout_break) {
        break;
      }
      for (double omega_raw : omega_values) {
        const double elapsed_ms =
          std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start).count();
        if (elapsed_ms > params_.max_compute_time_ms) {
          result.timeout = true;
          timeout_break = true;
          break;
        }
        ...
        const double vx = std::clamp(
          clampDelta(vx_raw, robot_velocity.linear.x, max_dvx), -result.vx_max, result.vx_max);
        const double vy = std::clamp(
          clampDelta(vy_raw, robot_velocity.linear.y, max_dvy), -result.vy_max, result.vy_max);
        const double omega = std::clamp(
          clampDelta(omega_raw, robot_velocity.angular.z, max_domega),
          -result.omega_max,
          result.omega_max);
