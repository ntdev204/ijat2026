#!/usr/bin/env python3
"""
Verify Continuous Context-Adaptive CA-NMPC dataset layout and metadata.
"""
...
class DatasetVerifier:
    ...
    def __init__(self, base_path: str, environment: str = "sim", min_runs: int = 20):
        self.base_path = Path(base_path).expanduser()
        self.raw_path = self.base_path / "raw" / environment
        self.environment = environment
        self.min_runs_per_scenario = min_runs
        self.scenarios = [
            "S1_open_zone",
            "S2_narrow_corridor",
            "S3_human_proximate",
    ...
    def _verify_scenario(self, scenario: str):
        ...
        run_dirs = []
        for controller_dir in scenario_path.iterdir():
        ...
        status = "COMPLETE" if valid_count >= self.min_runs_per_scenario else "INCOMPLETE"
        self._add_scenario_result(scenario, len(run_dirs), valid_count, status)

        print(f"  Total runs: {len(run_dirs)}")
        print(f"  Valid runs: {valid_count}/{self.min_runs_per_scenario}")
        print(f"  Status: {status}")

        if valid_count < self.min_runs_per_scenario:
            missing = self.min_runs_per_scenario - valid_count
    ...
    def _print_summary(self):
        ...
        total_required = len(self.scenarios) * self.min_runs_per_scenario
...
def main():
    ...
    parser.add_argument("--min-runs", type=int, default=20, help="Minimum valid runs per manual S1-S5 scenario")
