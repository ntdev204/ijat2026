#include "cca_nmpc_controller/cca_nmpc_controller.hpp"
...
void CCANMPCController::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(logger_, "Configuring CCANMPCController plugin: %s", name_.c_str());
  ...
  human_states_sub_ = node->create_subscription<ccanmpc_msgs::msg::HumanStates>(
    "/canmpc/humans", 10,
    std::bind(&CCANMPCController::humanStatesCallback, this, std::placeholders::_1));

  context_sub_ = node->create_subscription<ccanmpc_msgs::msg::Context>(
    "/canmpc/context", 10,
    std::bind(&CCANMPCController::contextCallback, this, std::placeholders::_1));
  ...
  solver_stats_pub_ = node->create_publisher<ccanmpc_msgs::msg::SolverStats>("~/solver_stats", 1);
  adaptive_bounds_pub_ = node->create_publisher<ccanmpc_msgs::msg::AdaptiveBounds>("~/adaptive_bounds", 1);
  ...
  try {
    package_prefixes.push_back(ament_index_cpp::get_package_prefix("cca_nmpc_controller"));
  ...
  RCLCPP_INFO(logger_, "CCANMPCController initialized successfully.");
}

void CCANMPCController::cleanup()
{
  RCLCPP_INFO(logger_, "Cleaning up CCANMPCController plugin: %s", name_.c_str());
...
void CCANMPCController::activate()
{
  RCLCPP_INFO(logger_, "Activating CCANMPCController plugin: %s", name_.c_str());
...
void CCANMPCController::deactivate()
{
  RCLCPP_INFO(logger_, "Deactivating CCANMPCController plugin: %s", name_.c_str());
...
void CCANMPCController::setPlan(const nav_msgs::msg::Path & path)
{
...
void CCANMPCController::getSpeedLimits(
...
geometry_msgs::msg::TwistStamped CCANMPCController::computeVelocityCommands(
  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  try {
    ...
    ccanmpc_msgs::msg::Context context_copy;
    ccanmpc_msgs::msg::HumanStates humans_copy;
    {
    ...
    RCLCPP_ERROR(logger_, "CCANMPCController exception: %s", ex.what());
    return cmd_vel;
  } catch (...) {
    RCLCPP_ERROR(logger_, "CCANMPCController unknown exception.");
    return cmd_vel;
  }
}

void CCANMPCController::setSpeedLimit(const double & speed_limit, const bool & percentage)
{
...
void CCANMPCController::humanStatesCallback(const ccanmpc_msgs::msg::HumanStates::SharedPtr msg)
{
  std::lock_guard<std::mutex> lock(data_mutex_);
  latest_human_states_ = *msg;
}

void CCANMPCController::contextCallback(const ccanmpc_msgs::msg::Context::SharedPtr msg)
{
  std::lock_guard<std::mutex> lock(data_mutex_);
  latest_context_ = *msg;
}
...
PLUGINLIB_EXPORT_CLASS(cca_nmpc_controller::CCANMPCController, nav2_core::Controller)
